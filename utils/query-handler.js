...
ee(Object.defineProperties(QueryHandler.prototype, {
	_resolveQueryValue: d(function (handler, value, query) {
		if (value != null) {
			if (value !== value.trim()) {
		...
		if (handler.ensure) {
			try {
				value = handler.ensure.call(this, value, query);
			} catch (e) {
	...
	resolve: d(function (query) {
		var resolvedQuery = this._handlers.reduce(function (resolvedQuery, handler) {
			var value = this._resolveQueryValue(handler, query[handler.name], query);
