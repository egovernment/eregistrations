...
This rule controls height of components that contain text, making shore that height is multiplication of 22px, and elements in side are place with correct VR.
...
***Form*** or ***ul*** wrapper element requires ***'form-elements'*** class in order for ***.radio*** and ***.hint*** elements to display properly.
...
In ***label*** simply label text is placed.  
In ***div.input*** input of significant type is placed, followed by ***span***'s:  
with ***statuses*** class for status marks;  
with ***error-message*** class for display of error text;  
with ***hint*** class for display of hint text. 
...
Responsive table flips its header to be first column for every row of table. From the code perspective, this can be achieved simply by adding property ***'responsive'*** set to value ***'true'*** on table. 
...
Can be used for display of warning message to user. Presents a white background with grey left-border and no radius.  
...
Can be used for display of white background with grey border and little radius container, that also have number of tabs at its top. Requires ***a*** elements with ***tab*** class for tabs placed as first children, and after requires a div element as main container for tab.  
...
***label-reg*** - simple grey label  
***label-reg.ready*** - orange label  
***label-reg.rejected*** - red label  
***label-reg.approved*** - green label  
***label-reg.info*** - blue label

Combined classes need to be used for colour labels to display properly: ***label-reg*** and type class, i.e.. ***ready***
...
Can be used on ***links***, ***buttons*** and ***inputs*** with ***submit*** type. Creates a grey button with total height of 44px. Can be combined with colour changing classes:  
***button-main-success*** - green colour button
***button-main-error*** - red colour button

Example can be found [here](https://github.com/egovernment/eregistrations/blob/master/view/prototype/revision.js#L114).

#### Post button

Post button component is a combination of elements created out of postButton() function. Markup created consists of ***form*** and ***submit*** type ***button***.  
...
Resource link is used for ***a*** elements, and displays it as a light grey button. 
...
Inside of this wrapper, ***div*** containing ***a*** elements for navigation (***previous*** and ***next*** classes), and a ***span*** with ***current-index*** for display of current file number.
...
For dialogs, also whole page background is changed. To achieve that, a ***div** element with ***modal-courtain*** is introduced. It need to be placed in markup, just as a empty div. ***modal-courtain*** requires ***dialog-modal-on*** class to be set on ***body*** element, for *courtain* to be displayed.

Markup for basic dialog consists of ***dialog*** main element.  
...
Inventory dialog is displayed for non-submitted user, in first step of submission - *guide*. Rules for sign up dialog are placed in *dialog-inventory.css* file. This dialog has ***dialog-inventory*** class. Main content of this dialog is a form, and most rules in its css file correspond to this element. 
