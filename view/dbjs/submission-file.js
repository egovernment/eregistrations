...
module.exports = Object.defineProperties(db.SubmissionFile, {
	inputOptions: d({
		render: function (options) {
			...
			this.control.addEventListener('change', function () { errorTxt.data = ""; });
			return dom;
		},
		renderItem: function (file) {
			...
			remove = isNested(file) ? file._destroy_.bind(file) : this.removeItem.bind(this, data.dom);
			...
			itemDom = _if(file._name, el('div', { class: 'file-thumb' },
				...
				data.control = el('input', { type: 'hidden', name: this.name, value: file.__id__ }),
				el('div', { class: 'file-thumb-actions' },
					...
					el('span', { class: 'file-thumb-action', onclick: remove },
