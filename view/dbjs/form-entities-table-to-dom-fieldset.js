		...
		customizeData.arrayResult = [
			...
			resolvent.formResolvent
				? ns.form({
					...
					autoSubmit: true,
					method: 'post'
				}, resolvent.formResolvent, ns.p({
					class: ['submit', _if(not(eqSloppy(this.getResolventObservable(), null)), 'completed')]
				}, ns.input({ type: 'submit', value: _("Submit") })))
				: undefined,
			_if(!this.resolventProperty || not(this._isUnresolved), div(
				...
				ns.div({ class: 'entities-overview-table-wrapper', id: resolvent.affectedSectionId },
					ns.table(
						...
						ns.tbody({
							onEmpty: ns.tr(ns.td({ colspan: this.entities.size + 2 }, this.onEmptyMessage))
						}, tableData, function (entityObject) {
